...
abstract class JCMModel extends Model {
  ...
  protected function transliterateString($txt) {
    ...
    $special = "$+!*'(),"; // _ .-
    $nic = [];
    foreach (str_split($unsafe . $reserved . $special) as $k => $w) {
      $nic[$w] = '';
    }
    $nic[' '] = '-';
    $nic[' - '] = '-';
    ...
    return strtolower(strtr($txt, $nic));
  }
  ...
  public function getUrlAttribute($v) {
    return $this->transliterateString($v);
  }

  public function getKeywordsAttribute($v) {
    return $this->extractXML($v);
  }
  ...
  public function getTrescAttribute($v) {
    return html_entity_decode($this->extractXML($v));
  }
